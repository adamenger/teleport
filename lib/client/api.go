...
func (tc *TeleportClient) startPortForwarding(ctx context.Context, nodeClient *NodeClient) {
	...
	if len(tc.Config.RemoteForwardPorts) > 0 {
		for _, fp := range tc.Config.RemoteForwardPorts {
			...
			socket, err := net.Dial("tcp", sourceAddr)
			log.Printf("Dialer connected to: %v", sourceAddr)
			log.Printf("Destaddr set to: %v", destAddr)
			if err != nil {
				log.Errorf("Failed to bind to %v: %v.", sourceAddr, err)
				continue
			}
			go nodeClient.remoteListenAndForward(ctx, socket, destAddr)
		}
	}
